...
from library_api.schemas import UserCreate, User
from library_api.services import user as user_service
from library_api.db import get_db

router = APIRouter()

@router.post("/register", summary="Регистрация пользователя", description="Этот маршрут позволяет зарегистрировать нового пользователя.")
...
@router.post("/login", summary="Авторизация пользователя", description="Этот маршрут позволяет пользователю войти в систему.")
